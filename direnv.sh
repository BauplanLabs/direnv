...
echo "direnv+op: running ${OLDPWD}/.envrc"
...
if [[ ! -z "${DIRENV_USE_FILE}" ]]; then
  ...
  if [ ${#envFiles[@]} -eq 0 ]; then
    echo "direnv+op: skipping ${RC_PATH}${DIRENV_USE_FILE} file not found"
    exit 0
  fi
else
  ...
  # Exit if the array is empty
  if [ ${#envFiles[@]} -eq 0 ]; then
    echo "direnv+op: .env* files not found in ${RC_PATH}"
    exit 0
  fi

  # Ask the user to pick one if there are multiple files
  if [ ${#envFiles[@]} -gt 1 ]; then
    echo "direnv+op: multiple files found in ${RC_PATH}:"
...
if [[ ! -f "${selFile}" ]]; then
  echo "direnv+op: file ${selFile} not found"
  exit 0
fi

# Now we need to read its content
echo "direnv+op: loading $selFile"
CONTENT=$(cat "$selFile")
